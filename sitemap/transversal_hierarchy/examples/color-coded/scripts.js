...
(function($){
  ...
  $(function() {
    ...
    var datascource = {
      ...
      'children': [
        { 'name': 'Bo Miao', 'title': 'department manager', 'className': 'essai1',
          'children': [
	...
	test.on('click', '.node', function() {
		
		var title = $( this ).children(".title");
		if (typeof title.data( "toogleColor" ) === 'undefined') {
		  title.data( "toogleColor", { couleur: "red", oldColor: title.css("color") } );
		  title.css({"color" : title.data( "toogleColor" ).couleur});
		}else{
			var toogCol = title.data( "toogleColor" );
			var tmp = toogCol.oldColor;
			toogCol.oldColor = toogCol.couleur;
			toogCol.couleur = tmp;
			title.css({"color" : toogCol.couleur});
		}
	});
